...
// тут реквест нужен, чтобы с бека получить список всех ботов. и в пайлоаде они храниться будут da ebanii rrrrrrot
export const selectAllBotsAction = (): BotsActionTypes => {
    return {
        type: SELECT_ALL_BOTS,
    };
};

export const selectAllBots = () => {
    // тут отдельный реквест будет, пока что опиши все остальное, или не можешь так? nu ya tipa rot ebalreturn getRequest()
};
